...
from sistema.models import Pousada as Hotel, TransacaoFinanceira
from .models import CategoriaProduto, Fornecedor, Produto, Compra, DocumentoCompra, ItemCompra, MovimentoEstoque, ProdutoConsumo
...
def criar_produto(request):
    ...
    hotel = pousada_ctx['active_pousada']
...
def excluir_produto(request, produto_id):
    ...
    return HttpResponseRedirect(reverse('estoque:dashboard'))
...
def criar_categoria(request):
    ...
    if request.method == 'POST':
        ...
        if request.headers.get('HX-Request') == 'true':
            ...
            response['HX-Redirect'] = reverse('estoque:dashboard')
            return response
        return redirect('estoque:dashboard')
...
def cancelar_compra(request, compra_id):
    ...
    with transaction.atomic():
        ...
        if compra.pago:
            ...
            TransacaoFinanceira.objects.filter(pousada=hotel, tipo='DESPESA', descricao=desc).delete()
            compra.pago = False
...
def reverter_movimento(request, movimento_id):
    ...
    if movimento.tipo != 'saida':
        ...
        if request.headers.get('HX-Request') == 'true':
            return render(request, 'estoque/partials/toast_message.html', {
                'mensagem': msg,
                'tags': 'error'
            })
    ...
    if request.headers.get('HX-Request') == 'true':
        response = render(request, 'estoque/partials/toast_message.html', {
            'mensagem': msg,
            'tags': 'success'
        })
        response['HX-Trigger'] = 'refreshEstoque'
        return response
...
def dashboard(request):
    ...
    produtos = Produto.objects.filter(hotel=hotel, ativo=True).select_related('categoria')
    if search_query:
        produtos = produtos.filter(Q(nome__icontains=search_query) | Q(codigo_barras=search_query))
    if categoria_id:
        produtos = produtos.filter(categoria_id=categoria_id)
        
    compras = Compra.objects.filter(hotel=hotel).select_related('fornecedor').prefetch_related('itens', 'itens__produto')
    if fornecedor_id:
        compras = compras.filter(fornecedor_id=fornecedor_id)
        
    movimentos = MovimentoEstoque.objects.filter(hotel=hotel).select_related('produto', 'criado_por').order_by('-criado_em')[:50]
    ...
    context = {
        **pousada_ctx,
        ...
        'categoria_selected_id': int(categoria_id) if categoria_id and categoria_id.isdigit() else None,
        'fornecedor_selected_id': int(fornecedor_id) if fornecedor_id and fornecedor_id.isdigit() else None,
        'hoje': date.today().strftime('%Y-%m-%d'),
        'itens_json': json.dumps([{
            'produto_id': '',
            'quantidade_raw': '1',
